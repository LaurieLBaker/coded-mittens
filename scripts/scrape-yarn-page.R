...
image_html <- paste0("<img src=\"", image, "\"", " alt=\"", yarn_name, "\" style=\"width:100px;height:100px;\">")
...
for(i in 1:nrow(yarn_table)){
  yarn_table$`Hex Code Blend`[i] <- get_top_color_blend(yarn_table$Image_URL[i])
  yarn_table$`Hex Code`[i] <- get_top_color(yarn_table$Image_URL[i])
